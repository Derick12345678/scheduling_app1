...
// Handle POST requests for adding, updating, or deleting information 
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    ...
    switch ($action) {
        case 'Add':
            ...
            $description = empty($_POST['appointment_description']) ? 'NULL' : "'" . $db->real_escape_string($_POST['start_time']) . "'";
            ...
            $appointmentsQuery = "
                INSERT INTO appointments (client_id, date, time, description)
                VALUES ($calendar_id, $event_date, $start_time, $description)
            ";
        ...
        case 'Update':
            ...
            $description = sanitizeInput($db, $_POST['appointment_description']);
            ...
            if ($start_time != 'NULL' && $event_date != 'NULL') {
                ...
                $updateQueryAppointments = "INSERT INTO appointments (client_id, date, time, description)
                VALUES ($id, $event_date, $start_time, $description)";
            ...
            <div class="modal-content">
                ...
                <form method="post">
                    <div class="modal-body">
                        ...
                        <h6 class="modal-title" id="addModalLabel">Appointment Information:</h6>
                        <br>
                        <div class="form-group">
                            <label>Date</label>
            ...
            <div class="modal-content">
                ...
                <form method="post">
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label>Start Time</label>
                            <input type="time" name="start_time" id="edit-start_time" class="form-control" step="3600">
                        </div>
                        <div class="form-group">
                            <label>Description</label>
                            <input type="text" name="appointment_description" id="edit-appointment_description" class="form-control">
                        </div>
                        <h6 class="modal-title" id="addModalLabel">Comments:</h6>
                        <div class="form-group">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <p><strong>Appointment Date:</strong> <span id="view-event_date"></span> <span id="view-start_time"></span></p>
                    <p><strong>Appointment Description:</strong> <span id="view-event_description"></span></p>
...
//function to prevent faulty appointments
document.addEventListener("DOMContentLoaded", function(){
    const form = document.querySelector("#editModal form");
    const eventDate = document.getElementById("edit-event_date");
    const startTime = document.getElementById("edit-start_time");
    const description = document.getElementById("edit-appointment_description");

    form.addEventListener("submit", function(e){
        const eventDate1 = eventDate.value;
        const startTime1 = startTime.value;
        const desc = description.value;
    
        if((eventDate1 && !startTime1) || (!eventDate1 && startTime1)){
            e.preventDefault();
            alert("Both 'Date' and 'Start Time' must be filled out together");
            return;
        }
        else if(desc && !startTime1){
            e.preventDefault();
            alert("Date and Start time must be filled for a description to be valid");
            return;
        }
    });
});
                ...
                if (response.success) {
                    ...
                    for (let key in comments) {
                        if (comments.hasOwnProperty(key)) {
                            ...
                            formattedComments += `<p><strong>${comment.user}</strong> at ${comment.comment_time} wrote:<br> ${comment.comment}</p>`;
                        }
                    }
                    $("#view-comments").html(formattedComments);
                    ...
                    $("#view-event_description").text(response.data.description);
                } else {
...
$(document).ready(function() {
    ...
    var table = $('#info_table').DataTable({
        ...
        pageLength: 25,  // Set the default number of entries to 25
		order: [[1, 'asc']], // Sorts by the second column (Title)
        columnDefs: [
            { orderable: false, targets: -1}, // Make the last column (Actions) non-sortable
        ]
    });
    ...
    $('#info_table').on('click', '.edit-btn', function() {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {   
                    $('#edit-id').val(id);
                    $('#edit-client_name').val(response.data.client_name);
                    $('#edit-event_date').val(response.data.date);
                    $('#edit-start_time').val(response.data.time);
                    $('#edit-address').val(response.data.address);
                    $('#edit-email').val(response.data.email);
                    $('#edit-city').val(response.data.city);
                    $('#edit-zip_code').val(response.data.zip_code);
                    $('#edit-phone').val(response.data.phone);
                    $('#edit-appointment_description').val(response.data.description)
